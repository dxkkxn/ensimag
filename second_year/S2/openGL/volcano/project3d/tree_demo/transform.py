...
class Camera:
    ...
    def move(self, direction):
        eye, target, up = self.convert_position_to_eye(self.position, self.pitch, self.roll, self.yaw)
        forward_direction = np.array([target[0] - eye[0], target[1] - eye[1], target[2] - eye[2]])
        right_direction = np.cross(forward_direction, up)
        speed = self.speed
        if direction == 0:
            ...
            self.position -= right_direction * speed
        elif direction == 3:
            self.position += right_direction * speed
        elif direction == 4:
    ...
    def convert_position_to_eye(self, position, pitch, roll, yaw):
        # Create rotation matrix
        quaternion = quaternion_from_euler(yaw, roll, pitch)
        rotation = quaternion_matrix(quaternion)[0:3, 0:3]

        # Calculate eye and up vectors
        eye = position
        up = np.dot(rotation, np.array([0, 0, 1]))

        # Calculate target vector
        target = eye + np.dot(rotation, np.array([0, 1, 0]))
    ...
    def projection_matrix(self, win_size):
        return perspective(self.fov, win_size[0] / win_size[1], self.near, self.far)