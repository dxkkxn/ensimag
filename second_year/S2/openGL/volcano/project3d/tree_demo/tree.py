...
def create_trees(nb_of_trees, complexity, heights):
    ...
    for _ in range(nb_of_trees):
        ...
        while not valid:
            ...
            if not in_circle(volcano_center, volcano_rayon, (x,y)) and \
                    not in_circle(water_center, water_rayon, (x,y)):
                valid = True
        root.add(Tree(complexity=complexity, transform=translate(x, 1, y) @ scale(.04, .04, .04)))
    return root
